...
from py360tools import Tile, TileStitcher
from skimage.metrics import mean_squared_error

from lib.main import Main
from lib.mainappif import ComboIf, GraphsIf, MainAppIf, PlayerIf
...
class NewChunk(MainAppIf, ComboIf):
    ...
    graphs_controls: GraphsControls
    ...
    def start_chunk(self):
        ...
        self.dectime = (time() - start_time)
        print(f'pre-process time is {self.dectime}ms')

        self.update_graphs_chunk()
        ...
        diff = (0 if self.dectime > self.frame_time
                else self.frame_time - self.dectime)
    ...
    def _update_tiles_seen_chunk(self):
        ...
        self.tiles_seen_ref = list(tiles_seen_ref)
    ...
    def _update_tile_stitcher(self):
        ...
        self.proj_stitcher_ref = TileStitcher(tiles_seen=self.tiles_seen_ref,
                                              proj_obj=self.proj_obj)
...
class VideoControls(NewChunk, MainAppIf, PlayerIf):
    button_dict: dict[str, ttk.Button | ttk.Checkbutton]
    _update_play_button: Callable
    ...
    def video_loop(self):
        ...
        if self.make_proj_frame() is None: return
        ...
        self.loop_id = self.app_root.after(diff, self.video_loop)

    def make_proj_frame(self):
        try:
            ...
            self.app_root.after(0, self.start_chunk)
            return None
    ...
    def get_mse(self):
        tiles = zip(self.proj_stitcher.tiles_seen,
                    self.proj_stitcher_ref.tiles_seen)
        mse_list = [mean_squared_error(tile.canvas, tile_ref.canvas)
                    for tile, tile_ref in tiles]

        proj_mse_value = float(np.average(mse_list))
        viewport_mse_value = mean_squared_error(self.viewport_frame,
                                                self.viewport_frame_ref)
        self.proj_mse_list.append(proj_mse_value)
